...
{
    public partial class FornitoriOld : UC<Fornitore, Categoria, Associazione_Categoria_Fornitore>
    {
        BindingList<Categoria> dataCombineFull = null;
        public FornitoriOld() : base()
        {
            InitializeComponent();
        ...
        protected override void editButton_Click(object sender, EventArgs e)
        {
            if (this.selectedIndex == -1)
            {
                MessageBox.Show("Nessun fornitore presente", "Gestione FornitoriOld", MessageBoxButtons.OK);
                return;
            }
            this.editButtons();
        }
