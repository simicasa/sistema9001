...
namespace ProgettoCMA
{
    public partial class Clienti : UC<Cliente>
    {
        ...
        private void clientiListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (this.newCliente != null)
            {
                DialogResult dr = MessageBox.Show("Cambiare cliente e perdere il nuovo?", "Gestione Clienti", MessageBoxButtons.YesNo);
                if (dr == DialogResult.Yes)
                {
                    this.inhibit = true;
                    this.data.Remove(this.newCliente);
                    this.dataSubset.Remove(this.newCliente);
                    //this.list.Items.Remove(this.newCliente);
                    this.orderList();
        ...
        {
            bool isNew = (this.newCliente == null) ? false : true;
        ...
        {
            this.inhibit = true;
            this.newCliente = new Cliente(-1, "", new Indirizzo(), "", "", "", "Nuovo Cliente", "");
            this.data.Add(newCliente);
            this.dataSubset.Add(newCliente);
            this.orderList();
            this.list.SelectedItem = newCliente;
            this.selectedIndex = this.list.SelectedIndex;
            this.editButtons();
            this.updateUI(this.newCliente);
            this.inhibit = false;
        }
